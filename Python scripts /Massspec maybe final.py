...
if __name__ == "__main__":
    ...
    if process:
        ...
        for pattern in process["patterns"]:
            ...
            pattern_results = {"pattern": truncated_pattern, "elements": {}}
            df_set = pd.DataFrame()
            csv_files = []
            ...
            for element in process["elements"]:
                ...
                for charge in sorted(process["file_assignments"][pattern][element].keys()):
                    if process["file_assignments"][pattern][element][charge]:
                        ...
                        avg_energy.append((df_set[f'E{col_idx}'] * df_set[f'IEDF{col_idx}']).sum() / df_set[f'IEDF{col_idx}'].sum())
                ...
                pattern_results["elements"][element] = {
                    ...
                    "avg_energy_60eV": avg_energy_60eV  # Add average energy (up to 60 eV) for each charge state
                }
