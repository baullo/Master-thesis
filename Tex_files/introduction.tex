...
Two diagnostics were operated simultaneously: a Langmuir probe measured ion current density, a quartz crystal microbalance tracked deposited mass in real time. Additionally an energy-resolving mass spectrometer determined charge-state-resolved ion energy distributions. Measurements were performed in both metallic mode (vacuum) and reactive mode (nitrogen background) to characterize the transition between regimes.\\

%need to check this and change bruh
%Results show that magnetic field strength increases ion flux by up to a factor of 8. In reactive mode, nitrogen pressure above 0.1~Pa reduces ion current by 50\% due to charge-exchange collisions while maintaining film stoichiometry. Direct correlation between ion current and deposited mass reveals a complex relation ship \textbf{need to write what it correlates if anything lol}. Energy-resolved mass spectrometry confirms that total ion energy (kinetic plus potential) remains in the 50--80~eV range suitable for room-temperature crystallization, though charge-state distributions shift toward lower ionization in reactive mode.\\
...
Titanium aluminum nitride (Ti$_{1-x}$Al$_x$N) coatings are widely used in cutting tools and wear-resistant applications because of their high hardness (25--35~GPa), thermal stability, and oxidation resistance \cite{paldey2003}. These properties depend on maintaining the metastable cubic B1 crystal structure, which provides superior mechanical performance compared to the thermodynamically stable wurtzite phase \cite{mayrhofer2006}. Traditional deposition methods require substantial substrate heating to achieve dense, crystalline coatings. Cathodic arc deposition offers a key advantage: highly ionized metal plasmas with intrinsic ion energies of 50--100~eV enable room-temperature crystallization through energetic condensation \cite{cathodic_arcs}.
...
\begin{enumerate}
	\item Characterize ion flux as a function of external magnetic field strength (0--0.25~T) using Langmuir probe measurements while simultaneously measuring mass deposition via QCM. Separately measure the ion energy distributions via energy-resolved mass spectrometry (ERMS).
	...
	\item Characterize the crystal structure (XRD), the film thickness (Profilometry), the film composition (EDX) and microstructure (SEM) of deposited TiAlN films to correlate plasma parameters with film properties.
\end{enumerate}

The experimental approach combines three diagnostics operated simultaneously: a biased Langmuir probe measures ion current density, a quartz crystal microbalance tracks mass deposition in real time, and an energy-resolving quadrupole mass spectrometer determines charge-state-resolved ion energy distributions. This multi-diagnostic strategy, applied across a systematic parameter space, enables separation of ion flux effects from ion energy effects while extending the Unutulmazsoy/Kalanov framework to reactive mode operation and the industrially relevant TiAlN system.
