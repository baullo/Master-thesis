...
In reactive mode, nitrogen incorporation fundamentally alters film growth. The appearance of sharp cubic TiAlN peaks at 0.1~Pa and their continued sharpening at higher pressures reflects progressive crystallization driven by the chemical driving force for nitride formation combined with energetic ion bombardment. The complete disappearance of the hexagonal TiAl peak indicates full conversion rather than phase coexistence.
...
\subsection{Spatial Film Thickness Gradient}

XRR measurements reveal a systematic 5--8.5~nm thickness difference between the QCM and ion current probe positions (Table~\ref{tab:xrr_results}), consistent with profilometry observations. This gradient arises primarily from the plasma transport through the curved macroparticle filter rather than the initial cathode spot emission. While the cathode spot generates plasma with a cosine-like angular distribution normal to the cathode surface \cite{boxman1992,cathodic_arcs}, the subsequent passage through the 90$^\circ$ magnetic filter fundamentally reshapes the spatial distribution of the plasma beam exiting toward the substrate region \cite{boxman1992,anders1999filter}.\\

The magnetic filter acts as a plasma optical system, guiding electrons along curved magnetic field lines while ions follow electrostatically to maintain quasineutrality \cite{anders1999filter}. The plasma transport efficiency through curved filters depends sensitively on the magnetic field geometry, filter duct dimensions, and the charge-to-mass ratio of the plasma species \cite{boxman1996}. Different ion species and neutrals may follow slightly different trajectories through the filter, leading to spatial variations in composition and flux at the substrate plane. Additionally, collisions with background gas can alter ion trajectories within the filter, though this effect appears minimal here since the gradient remains consistent across all nitrogen pressures (0--0.3~Pa).\\

At the experimental pressures employed, the mean free path for elastic collisions ($\lambda \sim 1$--10~cm) is comparable to or larger than the substrate dimensions, so gas-phase scattering between the filter exit and substrate produces some broadening but does not fundamentally alter the flux distribution established by the filter geometry. The pressure independence of the gradient confirms that filter geometry, not gas-phase collisions, dominates the spatial distribution.


\subsection{Mass Density and Structure-Zone Model}

XRR measurements show that nitrogen incorporation increases film density from 4.00~g/cm$^3$ (metallic) to 4.65--4.80~g/cm$^3$ (reactive), approaching the reference value of 4.8~g/cm$^3$ for cubic TiAlN. This 16--20\% density increase reflects two synergistic mechanisms operating in the Anders structure-zone model framework \cite{anders2010szm}.\\

First, the chemical driving force for nitride formation (formation enthalpies approximately $-$200~kJ/mol for TiN and AlN \cite{paldey2003}) promotes thermodynamically stable cubic phases with high atomic packing density. The strong Ti-N and Al-N covalent bonds create a rigid framework that resists void formation during growth. Second, energetic ion bombardment (30--60~eV total energy from ERMS) provides sufficient energy for adatom relocation and defect annealing through subplantation and collision cascades \cite[Chap.~8.1--8.2]{cathodic_arcs}.\\

The combination places the films in Zone~T or Zone~2 of the Anders model, characterized by dense, well-crystallized structures even without external substrate heating. In metallic mode, insufficient driving force for crystallization leaves the film in Zone~1 despite comparable ion energies, demonstrating that chemical bonding and ion bombardment act cooperatively rather than independently.\\

The slight density reduction at 0.3~Pa (4.65~g/cm$^3$) compared to 0.1~Pa (4.80~g/cm$^3$) may indicate over-nitriding or formation of secondary phases. XRD shows the strongest cubic reflections at 0.1~Pa, supporting the interpretation that this pressure represents an optimal balance between nitrogen supply and film crystallization.

\section{Correlation Between Plasma Parameters and Film Properties}


\section{Comparison with (V,Al)N System and Limitations}
