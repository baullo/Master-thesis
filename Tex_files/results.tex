...
\begin{subfigure}[b]{\textwidth}
  ...
  \label{fig:3Dion}
\end{subfigure}
\caption[3D visualization of mass flux and ion current]{Three-dimensional parameter space mapping of (a) mass deposition rate and (b) ion current from filtered cathodic arc plasma. Both quantities increase with magnetic field strength (enhanced confinement) and decrease with distance (plasma expansion), while exhibiting complex non-monotonic pressure dependence. Surface colors represent measured values across 64-pulse averaging at 5 Hz. These global trends guide the detailed cross-sectional analyses in subsequent figures.}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption[Mass and ion current vs. distance]{Distance scaling of (a) mass deposition rate and (b) ion current at maximum magnetic confinement (0.25 T). Mass deposition follows approximate $r^{-2}$ dilution in metallic mode with only moderate reactive gas effects. In contrast, ion current shows dramatic pressure-dependent suppression at short distances, indicating charge-exchange collisions preferentially affect measured current rather than total deposited mass. This divergence diminishes beyond 14 cm as plasma expansion reduces collision rates.}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption[Mass and ion current vs. magnetic field]{Magnetic field scaling at intermediate distance (14 cm) reveals divergent responses to plasma confinement. (a) Mass deposition rate increases uniformly with field strength across all pressures, demonstrating that improved magnetic guiding enhances total flux delivery regardless of background gas. (b) Ion current exhibits strong pressure bifurcation: metallic mode shows continuous enhancement while reactive conditions plateau or decrease at high fields, suggesting that increased confinement amplifies charge-exchange collision rates in nitrogen-containing plasmas.}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption[Mass and ion current vs. nitrogen pressure]{Nitrogen pressure dependence at 14 cm with 0.25 T confinement. (a) Mass deposition rate exhibits non-monotonic behavior with a local maximum near 0.05~Pa, reflecting competing effects of reactive species incorporation and scattering losses. (b) Ion current behaves similarly with the local maximum near 0.025~Pa}
...
Potential energies scale directly with charge state, with Ti showing $E_{\rm pot} \approx 38$~eV and Al $E_{\rm pot} \approx 26$~eV, remaining nearly constant across the measurement range. Kinetic energies are in the range of 20--30~eV for both species, with a slight decreasing trend with distance due to minor collisional thermalization. Importantly, total ion energies ($E_{\rm kin} + E_{\rm pot}$) exceed 40~eV at all distances, well above the $\approx$ 30~eV threshold for subplantation-driven densification \cite{lifshitz1990subplantation}. This confirms that energetic condensation should remains accessible across the entire 10--20~cm range without substrate heating.

\begin{figure}[h!]
	\centering
	\includegraphics[width=\linewidth]{"Figures/results 2/Distance_0.25T_0.0Pa"}
	\caption[ERMS distance dependence in metallic mode]{Ion energy distributions in metallic mode demonstrate minimal thermalization over 10--20 cm transport. Mean charge states (Q approx 2.0 for Al, 2.7 for Ti) remain constant, indicating negligible charge-exchange in vacuum. Both potential energy ($qV_p$) and kinetic energy components stay elevated (total >50 eV), confirming that ions retain their cathode spot derived energies during drift through the curved filter. This baseline establishes reference values for comparison with reactive mode degradation in subsequent figures.}
	\label{fig:ERMS_distance0Pa}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\linewidth]{"Figures/results 2/Distance_0.25T_0.3Pa"}
	\caption[ERMS distance dependence in reactive mode]{Reactive gas introduces systematic charge-state reduction and energy degradation. Compared to metallic baseline (Fig. \ref{fig:ERMS_distance0Pa}), Al and Ti charge states decrease 50\% through successive charge-exchange collisions with N$_2$. Kinetic energies exhibit both reduced mean values and increased scatter, indicating collisional thermalization partially converts directed motion into random thermal velocity. Potential energy ($qV_p$) decreases proportionally to charge state. Despite these losses, ions retain sufficient energy (>30 eV) for high quality film growth at 14 cm, the distance selected for depositions.}
...
\begin{figure}[h!]
	...
	\caption[ERMS pressure dependence at 10 cm]{Pressure dependent evolution of metal ion energetics at short distance (10 cm). Charge states decrease monotonically with $N_2$ pressure as expected from increasing charge exchange probability. Kinetic energy shows unexpected non-monotonic behaviour with maximum near 0.1 Pa, potentially indicating competing effects of initial acceleration in the dense filter region versus subsequent thermalization during transport. Potential energy tracks charge state reduction.}
...
\begin{figure}[h!]
	...
	\caption[ERMS pressure dependence at 14 cm]{Pressure dependence at deposition distance (14 cm) shows similar trends to 10 cm data but with reduced absolute energies due to longer transport path. The systematic charge-state reduction with pressure directly impacts available potential energy for film growth, while kinetic energies remain in the 20--40 eV range optimal for dense coating formation. This distance was selected for film depositions (Section~\ref{sec:film_char}).}
...
\begin{figure}[ht]
	...
	\end{subfigure}
	\caption[Plasma plume photographs]{Visible emission reveals distinct plasma morphologies. (a) Reactive mode (0.3~Pa N$_2$): Nitrogen collisions produce broader, diffuse plume with orange-red glow from N$_2$ excitation. (b) Metallic mode (vacuum): Tight confinement shows blue-white continuum from highly ionized Ti/Al species with minimal scatter. The dramatic visual difference directly reflects the collision dominated versus collisionless transport regimes quantified by probe measurements. (Exposure settings f/1.7, 1/100~s, ISO 90)}
...
\begin{table}[h]
	\centering
	\caption[Profilometry film thickness measurements]{Profilometry-derived thickness distribution confirms QCM predictions. Films deposited at 14~cm show expected radial thickness variation consistent with plasma beam divergence and QCM/ion probe positions during characterization measurements. Good agreement between profilometry and in-situ QCM measurements (Table~\ref{tab:xrr_results}) validates the real-time monitoring approach and demonstrates deposition uniformity within the central measurement zone. Step heights were measured using stylus profilometry after selective masking.}
...
\begin{figure}[h]
	...
	\caption[XRD patterns for varying nitrogen pressure]{Phase evolution with nitrogen incorporation reveals pressure-dependent crystallographic transitions. Metallic film (0~Pa) shows hexagonal close-packed Ti--Al alloy with (002) texture at 38.8$^\circ$. Introduction of nitrogen (0.1--0.3~Pa) produces rock-salt structure (Ti,Al)N with (111), (200), (220) reflections, indicating successful nitride formation. Peak intensities and positions shift systematically with pressure, reflecting changing N content (quantified by EDX, Fig.~\ref{fig:edx_composition}) and Al incorporation limits. Substrate peaks (52--57$^\circ$) belong to Silicon. Grazing incidence geometry ($\omega=1^\circ$) enhances thin-film sensitivity while minimizing substrate contribution.}
...
\begin{table}[h]
	\centering
	\caption[XRR derived thickness]{X-ray reflectometry provides independent thickness validation and reveals density variations. Mass densities extracted from critical angle and Kiessig fringe spacing range from 4.00 to 4.8~g~cm$^{-3}$, reaching theoretical TiAlN bulk density (4.8~g\,cm$^{-3}$), indicating residual porosity or columnar microstructure.}
...
A systematic thickness gradient is observed between the two substrate positions, with the ion current location consistently showing 5--8.5~nm lower thickness compared to the QCM position. This corresponds to a reduction of 8--18\%, with film 13 exhibiting the most pronounced gradient at 8.5~nm (18\%). This spatial non-uniformity is consistent with the profilometry measurements presented in Table~\ref{tab:profilometry_results}, where thickness differences of 5--18~nm between the two positions were observed across all samples.\\

The XRR thickness values are on average a bit small than the ones measured with profilometry, with differences typically within 10\%. This agreement validates both measurement techniques and confirms that the observed gradient is a real spatial variation in the amount of deposited particles at each measurement device and can therefore be accounted and corrected for.\\

\paragraph{Mass Density Evolution}

The introduction of nitrogen substantially increases the film mass density from 4.00~g/cm$^3$ in metallic mode to 4.65--4.80~g/cm$^3$ in reactive mode. This density increase reflects the transition from a loosely packed metallic TiAl structure to a dense cubic TiAlN nitride phase. The measured densities in reactive mode closely approach the reference value of 4.8~g/cm$^3$ for stoichiometric cubic TiAlN \cite{ICDD_TiAlN}, indicating nearly complete densification and crystallization.\\
...
\begin{table}[h]
	\centering
	\caption[Film composition from EDX]{Composition analysis reveals systematic nitrogen incorporation and preferential Ti loss. Nitrogen content increases monotonically from 0 to 48~at.\% as pressure rises from 0 to 0.3~Pa, approaching stoichiometric (Ti,Al)N. Ti/Al ratio shifts from 4:1 toward 3:1 at high N$_2$, indicating preferential Ti resputtering or reduced sticking probability under nitrogen bombardment. Effective molar mass tracks composition changes, ranging from 43~g~mol$^{-1}$ (metal) to 32~g~mol$^{-1}$ (nitride). These values enable accurate conversion between QCM mass and atomic flux (Section~\ref{sec:flux_calculations}).}
	\label{tab:edx}
...
\begin{figure}[h!]
	...
	\caption[SEM surface morphology]{Surface morphology evolution with nitrogen pressure. (a) Metallic film (0~Pa) exhibits swelling features with macroparticles, characteristic of low-energy deposition in Zone 1 regime. (b) Dual imaging of individual macroparticle (50,000$\times$): secondary electron image (left, topography-sensitive) shows macroparticle while backscattered electron image (right, composition-sensitive) shows no atomic number contrast, confirming the macroparticle has identical elemental composition as the surrounding TiAlN film. (c) Reactive TiAlN (0.3~Pa) surface shows reduced amount of features, indicating better filtering in the presence of nitrogen.}
...
\begin{figure}[h]
	...
	\caption[Ion and atomic flux vs. pressure]{Ion flux and atomic flux as functions of nitrogen pressure at 14~cm distance with 0.25~T magnetic field. Ion flux (blue, left axis) increases from $(0.7 \pm 0.25) \times 10^{17}$ in metallic mode to $(1.0 \pm 0.4) \times 10^{17}$~ions$\cdot$cm$^{-2}$$\cdot$s$^{-1}$ at 0.2~Pa, consistent with the ion current behavior. Atomic flux (orange, right axis) exhibits non-monotonic pressure dependence with a maximum of $(1.15 \pm 0.05) \times 10^{17}$~atoms$\cdot$cm$^{-2}$$\cdot$s$^{-1}$ near 0.2~Pa. Error bars on ion flux represent combined variation ion flux within a pulse from probe current measurements and ERMS charge-state determination.}
	\label{fig:flux_vs_pressure}
\end{figure}


The ion flux shows a increase with nitrogen pressure, from $(0.7 \pm 0.2) \times 10^{17}$~ions$\cdot$cm$^{-2}$$\cdot$s$^{-1}$ in metallic mode to $(1.0 \pm 0.4) \times 10^{17}$~ions$\cdot$cm$^{-2}$$\cdot$s$^{-1}$ at 0.2~Pa before decreasing slightly to $(1.0 \pm 0.4) \times 10^{17}$~ions$\cdot$cm$^{-2}$$\cdot$s$^{-1}$ at 0.3~Pa. The large error bars ($\pm 30$--$50\%$) reflect combined variation ion flux within a pulse from the ion current probe measurements and ERMS charge-state determination. The overall trend mirrors the ion current behavior observed in Figure~\ref{fig:pressure_0.25T_mass/ion}b, where current initially increases from 0 to 0.1~Pa due to additional nitrogen ion production before saturating at higher pressures.\\
