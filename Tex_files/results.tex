...
The experimental parameter space spanned three variables: distance from the macroparticle filter (10--20~cm), applied magnetic field strength (0--0.25~T), and nitrogen background pressure (0--0.3~Pa), with all permutations measured as listed in Table~\ref{tab:parameters}. At this stage, the raw quantities (deposited mass in nanograms/cm$^2$s and ion current in milliamperes) are not directly comparable and only trends will be observed. Their relationship will be established through flux calculations in Section~\ref{sec:flux_comparison}.
...
\begin{subfigure}[b]{\textwidth}
  ...
  \label{fig:3Dion}
\end{subfigure}
\caption{Three-dimensional visualization of (a) deposited mass per pulse normalized by collection area and (b) pulse-averaged ion current as functions of distance (10--20~cm), magnetic field strength (0--0.25~T), and nitrogen pressure (0--0.3~Pa).}
...
In the absence of reactive gas, the plasma expansion and deposition dynamics are governed solely by geometric dilution and magnetic confinement. Figure~\ref{fig:metallic_mass/ion} presents deposited mass and ion current as functions of magnetic field strength for three representative distances.
\begin{figure}[h]
	\centering
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{Metallic case measurements showing (a) the deposited mass per pulse and area, and (b) the ion current, each plotted as a function of magnetic field strength. Data selected for three distances (10, 14, 20 cm), with error bars representing variation within pulses.}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{QCM and Ion Probe measurements showing (a) the deposited mass per pulse and area, and (b) the ion current, each plotted as a function of distance. Data shown for representative pressures, with error bars representing variation within pulses.}
	\label{fig:distance_0.25T_mass/ion}
\end{figure}

Both deposited mass and ion current decrease with distance, reflecting the natural expansion and dilution of the plasma plume. In the case for the mass, this decay is relatively gradual and follows an approximate $1/r^2$ dependence expected for free expansion \cite[Chap.~4.3]{cathodic_arcs}.\\
...
Figure~\ref{fig:mag_field_14cm_mass/ion} presents deposited mass and ion current as functions of magnetic field strength at a fixed intermediate distance (14~cm) for the same set of pressures.
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{QCM and Ion Probe measurements showing (a) the deposited mass per pulse and area, and (b) the ion current, each plotted as a function of Magnetic Field. Data shown for representative pressures, with error bars representing variation within pulses.}
...
These observations demonstrate that background gas pressure has a relatively minor influence on total deposited mass compared to distance or magnetic field, but significantly affects the charge-state composition of the arriving flux. This distinction motivates the final cross-section through the parameter space: examining pressure as the primary variable.

\subsection{Nitrogen pressure as a variable}\label{subsec:nitrogen_pressure}
Figure~\ref{fig:pressure_0.25T_mass/ion} presents deposited mass and ion current as functions of nitrogen pressure at maximum magnetic confinement (0.25~T) for three distances (10, 14, and 20~cm).
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{QCM and Ion Probe measurements showing (a) the deposited mass per pulse and area, and (b) the ion current, each plotted as a function of Pressure. Data shown for representative pressures, with error bars representing variation within pulses.}
