...
Before plasma investigations were performed, the functionality of the custom-built Ion current probe was verified by characterizing the voltage-current relationship and determining the appropriate negative bias voltage value for ion saturation range. The goal was to ensure that the probe operates in a regime where the collected current is dominated by ions, excluding the contributions from electrons.\\

The bias voltage measurements were performed without nitrogen in the chamber, at a fixed distance of 10~cm away from the filter, and with an EM-coil strength of 0.25~T. \\
...
\begin{figure}[h]
	...
	\caption[Ion current probe biasing tests]{Measured ion current vs. bias voltage, showing I-V fit $I=6.17(1-e^{-V/1.8})+0.023V$. Process conditions: no gas, 10~cm away from the macroparticle filter, 0.25~T EM-coil strength.}
