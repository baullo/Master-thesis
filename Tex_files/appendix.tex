...
QCM frequency measurements were recorded before and after each set of pulses. The deposited mass was calculated using the Sauerbrey equation (Equation~\ref{eq:sauerbrey}). The mass flux was obtained by dividing by the number of pulses and the crystal area.
...
\begin{table}[h]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		QCM mass & 0.01\% & Instrument precision \\
		Ion current & 33--68\%  & Plasma variability \\
		Mean charge state & 14--24\%  & Charge state spread \\
		Mass flux & 0.01\% & QCM precision \\
		Ion flux & 40--70\% & Ion current + charge state \\
...
\chapter{Supplementary material}\label{sec:additional plots}
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption[Additional distance measurements]{Additional plot for Section \ref{subsec:distance}. Distance from the filter exit dependence of (a) mass flux and (b) ion current  at 0~T magnetic field strength, measured at nitrogen pressures of 0, 0.1, 0.2 and 0.3~Pa}
...
\begin{figure}[h!]
	...
	\caption[Magnetic field correlation with charge state]{Mean charge state as a function of magnetic field strength for Ti and Al ions under representative conditions: (a) 10 cm distance in metallic mode (0 Pa), (b) 10 cm distance in reactive mode (0.3 Pa), and (c) 14 cm distance in reactive mode (0.1 Pa).}
...
\begin{itemize}[noitemsep]
	...
	\item \href{https://github.com/baullo/Junk_Paul-Masterthesis-Code/tree/main/spreadsheets}{Data sets} created and used for the Thesis
\end{itemize}
