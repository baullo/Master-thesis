...
\begin{equation}
	\Phi_{\text{ion}} = \frac{I_{\text{ion}} \cdot N_A}{A \cdot \bar{Q} \cdot F}
	\label{eq:ion_flux_mass}
\end{equation}
where $N_A$ is the Avogadro number, $A$ is the collection area of the ion probe, $\bar{Q}$ is the mean charge state and $F$ is Faraday's constant ($96 485 \frac{A}{mol}$).\\
