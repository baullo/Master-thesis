...
\begin{figure}[h]
	...
	\caption{Example voltage waveform measured across the 400~$\Omega$ resistor 
...
\begin{table}[h]
	\centering
	\caption{Mass-to-charge ratios measured for each ion species and charge state.}
	\label{tab:mq_ratios}
...
The ion flux and total deposited flux are key parameters for understanding the correlation between plasma properties and film growth. These quantities are derived from combined measurements of the Langmuir probe, QCM, ERMS, and film composition analysis.
...
The total deposited flux $\Phi_{\text{total}}$ (atoms\,cm$^{-2}$\,pulse$^{-1}$) includes contributions from both ions and neutrals (in reactive mode). This quantity is derived from the QCM mass deposition measurements:
\begin{equation}
	\Phi_{\text{total}} = \frac{\Delta m \cdot N_A}{A_{\text{QCM}} \cdot N_{\text{pulses}} \cdot M_{\text{eff}}}
\end{equation}

where $\Delta m$ is the mass change measured by QCM over $N_{\text{pulses}}$ pulses, $A_{\text{QCM}} = 0.5027$~cm$^2$ is the active crystal area, $N_A = 6.022 \times 10^{23}$~mol$^{-1}$ is Avogadro's constant, and $M_{\text{eff}}$ is the effective molar mass of the deposited material.


The effective molar mass $M_{\text{eff}}$ depends on the film composition, which was determined by energy-dispersive X-ray spectroscopy (EDX, Section~\ref{sec:EDX}). For films deposited in metallic mode, $M_{\text{eff}}$ reflects the Ti:Al ratio of the cathode composition. For reactive mode deposition, $M_{\text{eff}}$ accounts for nitrogen incorporation according to the measured TiAlN stoichiometry. The film density $\rho_{\text{film}}$ required to relate mass deposition rate to film thickness was determined by X-ray reflectometry (XRR, Section~\ref{sec:XRR}).
...
\begin{figure}[h]
	...
	\caption{Interior view of the SEM sample chamber showing the sample holder stage and detector configuration used for imaging and EDX analysis.}
