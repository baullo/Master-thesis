...
An in-house-built ion collector probe was designed to measure the ion current density in ion saturation mode. The probe consisted of a 5~mm diameter copper rod machined to form a cylindrical collection surface with a nail-head geometry. This design ensured a well-defined collection area while minimizing perturbation of the plasma flow. The probe body was wrapped in Kapton tape for electrical insulation from the electrically grounded surrounding aluminum mounting assembly (Figure~\ref{fig:holder assembly}).\\

To ensure full ion collection and suppress electron current, the probe was negatively biased at $V_b = -80$~V, as determined through bias voltage characterization measurements (Section~\ref{sec:bias_characterization}). The bias voltage was supplied through a protection circuit consisting of a 22~nF capacitor and 1~k$\Omega$ resistor, which acted as a low-pass filter to smooth the bias voltage and minimize high-frequency ripple. Two Zener diodes (1~k$\Omega$ and 10~k$\Omega$) provided overvoltage protection for the 80~V bias supply against transient voltage spikes during arc ignition.


\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{"Figures/experimental methods/langmuirprobe"}
	\caption{In-house built ion collector probe wrapped in Kapton tape for electrical insulation from the aluminum assembly holder. The probe includes an attachment point for a screw terminal connector, enabling connection to the ion probe circuit.}
	\label{fig:langmuirprobe}
\end{figure}

The probe circuit (Figure~\ref{fig:probe_circuit}) converts the collected ion current to a voltage signal suitable for oscilloscope measurement. The ion current flows through a 400~$\Omega$ measurement resistor, producing a voltage drop $V_{\text{measured}}$ that is directly proportional to the ion current. A 0.5~$\mu$F capacitor in series with the measurement resistor acts as a high-pass filter with a cutoff frequency of:
\begin{equation}
	f_c = \frac{1}{2\pi RC} = \frac{1}{2\pi \cdot 400\,\Omega \cdot 0.5 \times 10^{-6}\,\text{F}} \approx 795\,\text{Hz}
\end{equation}

This filter blocks DC offsets and low-frequency noise while passing the plasma fluctuations associated with the 1~ms arc pulses (equivalent to 1~kHz). The RC time constant of this circuit is:
\begin{equation}
	\tau = RC = 400\,\Omega \cdot 0.5 \times 10^{-6}\,\text{F} = 200\,\mu\text{s}
\end{equation}

This time constant is significantly shorter than the pulse period (200~ms to 2~s), ensuring that the capacitor fully discharges between consecutive pulses. 
...
The processed voltage signal was recorded using a 20~MHz bandwidth oscilloscope \textcolor{red}{(Tektronix [MODEL], [sampling rate], [resolution])}. Figure~\ref{fig:ion_current_waveform} shows an example ion current waveform averaged over [NUMBER] pulses at a repetition rate of [X]~Hz. The time-averaged ion current during the pulse (0 to 1~ms) was calculated by integrating the voltage signal and dividing by the pulse duration.\\
%maybe picture of ion current waveform in appendix

The ion current density $J_i$ (A\,cm$^{-2}$) is calculated from the measured voltage as:
\begin{equation}
	J_i = \frac{V_{\text{measured}}}{A R}
\end{equation}
where $A = 0.1963$~cm$^2$ is the probe collection area (5~mm diameter) and $R = 400$~$\Omega$. The ion flux $\Gamma_i$ (ions\,cm$^{-2}$\,s$^{-1}$) is then determined by accounting for the average ion charge state $\langle Q \rangle$:
\begin{equation}
	\Gamma_i = \frac{J_i}{e \langle Q \rangle} = \frac{V_{\text{measured}}}{e \langle Q \rangle A R}
\end{equation}
where $e = 1.602 \times 10^{-19}$~C is the elementary charge. The average charge state $\langle Q \rangle$ was determined from energy-resolved mass spectrometry measurements (Section~\ref{sec:QMS}) for each experimental configuration.
...
A quartz crystal microbalance (QCM) was used to measure the deposited mass during pulsed cathodic arc deposition. The system consisted of an INFICON Cool Drawer\texttrademark\  with a single sensor in standard orientation, water-cooled to ensure thermal stability. A 14~mm diameter, 6~MHz AT-cut quartz crystal was operated with an SQM-160 controller for electronic readout.
...
Here $\rho_q = 2.649$~g\,cm$^{-3}$ is the quartz density, $r = 0.7$~cm is the radius of the exposed crystal area, $N_{\text{AT}} = 166\,100$~Hz\,cm is the frequency constant of the AT-cut quartz, $F_q = 6$~MHz is the uncoated resonance frequency, and $\Delta F$ is the measured frequency shift.\\
...
To enable direct comparison between ion current and deposited mass, the Langmuir probe and QCM were mounted on the same movable aluminum assembly (Figure~\ref{fig:holder assembly}). The probe was positioned through a precision-milled pass-through hole, while the QCM was secured in a dedicated cutout and fixed via screws. This configuration ensured rigid mechanical alignment between the two diagnostics throughout all measurements.\\

The probe collection area ($A_{\text{probe}} = 0.196$~cm$^2$, diameter 5~mm) was smaller than but contained within the QCM active area ($A_{\text{QCM}} = 0.5027$~cm$^2$, diameter 8~mm). Both diagnostics were positioned as close as possible to each other to minimize spatial gradients between measurement locations. However, this design does not completely eliminate the effect of radial plasma density gradients, which are inherent to expanding cathodic arc plasmas \cite[Chap.~6.2]{cathodic_arcs}. The close proximity brings the measured flux and plasma conditions as close as possible, though small differences due to the radial gradient remained and were evident in the experimental results.\\
...
\subsection{Calculation of Ion Flux and Total Deposited Flux}
The ion flux and total deposited flux are key parameters for understanding the correlation between plasma properties and film growth. These quantities are derived from the combined measurements of the Langmuir probe, QCM,  ERMS and film composition.

\subsubsection{Ion Flux from Probe and ERMS Measurements}
...
\end{equation}
where $\langle Q \rangle$ is the average charge state determined from ERMS measurements. This combination of probe and mass spectrometer data accounts for the multiply charged nature of the cathodic arc plasma and enables accurate determination of the particle flux from the electrical current measurement.

\subsubsection{Total Deposited Flux from QCM and Film Characterization}

The total deposited flux $\Phi_{\text{total}}$ (atoms\,cm$^{-2}$\,pulse$^{-1}$) includes contributions from both ions and neutrals (in reactive mode). This quantity is derived from the QCM mass deposition rate:
\begin{equation}
	\Phi_{\text{total}} = \frac{\Phi_{\text{mass}} N_A}{M_{\text{eff}}} = \frac{\Delta m \cdot N_A}{A_{\text{QCM}} \cdot N_{\text{pulses}} \cdot M_{\text{eff}}}
\end{equation}
where $N_A = 6.022 \times 10^{23}$~mol$^{-1}$ is Avogadro's constant and $M_{\text{eff}}$ is the effective molar mass of the deposited material.\\

The effective molar mass depends on the film composition, which was determined by energy-dispersive X-ray spectroscopy (EDX, Section~\ref{sec:EDX}). $M_{\text{eff}}$ reflects the TiAlN stoichiometry determined by EDX analysis of the deposited films. The film density $\rho_{\text{film}}$ required to relate mass deposition rate to film thickness was determined by X-ray reflectometry (XRR, Section~\ref{sec:XRR}).\\

The ratio $\Phi_{\text{total}} / \Gamma_{\text{ion}}$ provides insight into the fraction of deposited material arriving as ions versus neutrals, which is particularly important for understanding reactive mode deposition where activated nitrogen species contribute significantly to film growth.
...
\begin{figure}[h]
	...
	\caption{Example XRR measurement and fit for a TiAlN thin film deposited on Si substrate. The oscillations (Kiessig fringes) arise from interference between reflections from the film surface and the film-substrate interface.}
	\label{fig:xrr_fit}
\end{figure}
