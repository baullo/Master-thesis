...
Plasma was generated using a water-cooled cathode composed of 75~wt\% Ti--25~wt\% Al (62.8~at\% Ti--37.2~at\% Al) with a diameter of 6.35~mm and 38.1~mm long. The arc power supply operated in pulsed DC mode, delivering up to 400~A at a pulse frequency of 0.2--5~Hz, and also powered the 90$^\circ$ curved macroparticle filter in series. An accelerator coil (EM-coil), capable of currents up to 700~A, was pulsed 200~$\mu$s before arc ignition to stabilize the magnetic field. The QCM and Langmuir probe were mounted on a custom adjustable mechanism screwed to the chamber floor at 10--20~cm from the filter exit, while the energy-resolving mass spectrometer (ERMS) was positioned using a linear feedthrough for external adjustment.\\
...
\subsection{Ion-Flux Probe- Langmuir probe}\label{sec:langmuir_probe}
...
All experimental data were processed using custom Python scripts to ensure consistency and reproducibility. An Excel spreadsheet served as a central logbook, with each experiment identified by a unique suffix. The logbook recorded experimental conditions such as date, distances, folder name, base pressure, working pressure, MFC flow rate, cryopump slider position, power supply voltages, pulse count, pulse length, and off time between pulses. 

Additionally, it included initial and final frequencies from the quartz crystal microbalance (QCM) to calculate the deposited mass and referenced the corresponding raw data files, such as oscilloscope waveforms saved as CSV files for ion current measurements.\\

A custom Python script was developed to streamline the data processing pipeline. This script matched each experiment in the logbook to its associated raw data files, averaged the ion current waveforms over multiple pulses to reduce noise, when this wasn't done by the oscilloscope beforehand.

Then it compiled the experimental parameters, calculated deposited mass, and processed ion current data into a unified dataset. This automated approach ensured that all data were processed uniformly and efficiently, facilitating subsequent analysis and interpretation.\\

For the analysis of quadrupole mass spectrometer (QMS) data, another custom Python script was employed. This script processed the ion energy distribution functions (IEDFs) by integrating raw data files from the QMS, applying a mass transmission correction to account for detection biases, and calculating key metrics such as average ion energy, mean charge state, and potential and kinetic energies for each element and charge state. 

The script also generated plots for each element and compiled detailed results into a structured CSV file, enabling comprehensive analysis of the ion energy distributions.\\




\begin{itemize}
    \item errorhandling if at all???
    \item let s see if anyting else
\end{itemize}
